...
class PlaceDetailsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        arguments?.getString(Constants.PLACE_KEY)
        fetchPlaceDetails()
    }

    private fun fetchPlaceDetails() = with(viewModel) {
    ...
    private fun processResponse(state: ScreenState<PlaceModel>?) = with(binding) {
        when (state) {
    ...
    private fun displayDetails(place: PlaceModel) = with(binding) {
